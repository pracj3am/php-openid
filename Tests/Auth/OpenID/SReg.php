...
class SRegURITest extends PHPUnit_TestCase {
    function test_is11()
    {
        $this->assertEquals(\Auth\OpenID\SREG_NS_URI_1_1,
                            \Auth\OpenID\SREG_NS_URI);
    }
}
...
class CheckFieldNameTest extends PHPUnit_TestCase {
    function test_goodNamePasses()
    {
        ...
        foreach ($Auth_OpenID_sreg_data_fields as $field_name => $desc) {
            $this->assertTrue(\Auth\OpenID\checkFieldName($field_name));
        }
    }

    function test_badNameFails()
    {
        $this->assertfalse(\Auth\OpenID\checkFieldName('INVALID'));
    }

    function test_badTypeFails()
    {
        $this->assertfalse(\Auth\OpenID\checkFieldName(null));
    }
}
...
class SupportsSRegTest extends PHPUnit_TestCase {
    function test_unsupported()
    {
        ...
        $this->assertfalse(\Auth\OpenID\supportsSReg($endpoint));
        $this->assertEquals(array(\Auth\OpenID\SREG_NS_URI_1_1,
                                  \Auth\OpenID\SREG_NS_URI_1_0),
                            $endpoint->checked_uris);
    }

    function test_supported_1_1()
    {
        $endpoint = new FakeEndpoint(array(\Auth\OpenID\SREG_NS_URI_1_1));
        $this->assertTrue(\Auth\OpenID\supportsSReg($endpoint));
        $this->assertEquals(array(\Auth\OpenID\SREG_NS_URI_1_1),
                            $endpoint->checked_uris);
    }
    ...
    {
        $endpoint = new FakeEndpoint(array(\Auth\OpenID\SREG_NS_URI_1_0));
        $this->assertTrue(\Auth\OpenID\supportsSReg($endpoint));
        $this->assertEquals(array(\Auth\OpenID\SREG_NS_URI_1_1,
                                     \Auth\OpenID\SREG_NS_URI_1_0),
                               $endpoint->checked_uris);
...
class FakeMessage {
    ...
    {
        $this->openid1 = false;
        $this->namespaces = new \Auth\OpenID\NamespaceMap();
    }
...
class GetNSTest extends PHPUnit_TestCase {
    ...
    {
        $ns_uri = \Auth\OpenID\SRegBase::_getSRegNS($this->msg);
        $this->assertEquals($this->msg->namespaces->getAlias($ns_uri), 'sreg');
        $this->assertEquals(\Auth\OpenID\SREG_NS_URI, $ns_uri);
    }
    ...
    {
        $this->msg->openid1 = true;
        $this->msg->namespaces->add(\Auth\OpenID\SREG_NS_URI_1_0);
        $ns_uri = \Auth\OpenID\SRegBase::_getSRegNS($this->msg);
        $this->assertEquals(\Auth\OpenID\SREG_NS_URI_1_0, $ns_uri);
    }

    function test_openID1Defined_1_0_overrideAlias()
    {
        foreach (array(true, false) as $openid_version) {
            foreach (array(\Auth\OpenID\SREG_NS_URI_1_0,
                           \Auth\OpenID\SREG_NS_URI_1_1) as $sreg_version) {
    ...
    function test_openID1DefinedBadly()
    {
        ...
        $this->assertTrue(\Auth\OpenID\SRegBase::_getSRegNS($this->msg) === null);
    }
    ...
    function test_openID1_sregNSfromArgs()
    {
        ...
        $m = \Auth\OpenID\Message::fromOpenIDArgs($args);

        $this->assertTrue($m->getArg(\Auth\OpenID\SREG_NS_URI_1_1, 'optional') == 'nickname');
        $this->assertTrue($m->getArg(\Auth\OpenID\SREG_NS_URI_1_1, 'required') == 'dob');
    }
}
...
class SentinelFakeMessage {
    function SentinelFakeMessage($test_case)
    {
        ...
        $this->message = new \Auth\OpenID\Message();
    }
...
class TestingReq extends \Auth\OpenID\SRegRequest {
    static function fromOpenIDRequest($thing, $test_case='\Auth\OpenID\SRegRequest')
    {
...
class SRegRequestTest extends PHPUnit_TestCase {
    ...
    {
        $this->assertTrue(\Auth\OpenID\SRegRequest::build(array('elvis')) === null);
    }
...
class SRegResponseTest extends PHPUnit_TestCase {
    function test_fromSuccessResponse_signed()
    {
        ...
        $sreg_resp = \Auth\OpenID\SRegResponse::fromSuccessResponse($success_resp);
...
class SendFieldsTest extends PHPUnit_TestCase {
    private function _test($uri)
    {
        ...
        $sreg_req = \Auth\OpenID\SRegRequest::build(array('nickname', 'email'),
                                                   array('fullname'));
        $req_msg = new \Auth\OpenID\Message($uri);
        $req_msg->updateArgs(\Auth\OpenID\SREG_NS_URI,
                             $sreg_req->getExtensionArgs());

        $req = new \Auth\OpenID\Request();
        $req->message =& $req_msg;
        ...
        $resp_msg = new \Auth\OpenID\Message($uri);
        $resp = new \Auth\OpenID\ServerResponse($req);
        ...
        $sreg_resp = \Auth\OpenID\SRegResponse::extractResponse($sreg_req, $data);
        $resp->addExtension($sreg_resp);

        // <- send id_res response

        // Extract the fields that were sent
        $sreg_data_resp = $resp->fields->getArgs(\Auth\OpenID\SREG_NS_URI);
        $this->assertEquals(
    ...
    function test()
    {
        foreach (array(\Auth\OpenID\OPENID1_NS,
                       \Auth\OpenID\OPENID2_NS) as $uri) {
            $this->_test($uri);
        }
    }
}
