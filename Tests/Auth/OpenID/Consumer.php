...
function mkSuccess($endpoint, $q)
{
    ...
    return new \Auth\OpenID\SuccessResponse($endpoint,
                                           \Auth\OpenID\Message::fromOpenIDArgs($q),
                                           $signed_list);
}

class FastConsumerSession extends \Auth\OpenID\DiffieHellmanSHA1ConsumerSession {
    function __construct($dh = null)
    {
        if ($dh === null) {
            $dh = new \Auth\OpenID\DiffieHellman(100389557, 2);
        }
...
function associate($qs, $assoc_secret, $assoc_handle)
{
    $query_data = parse($qs);
    ...
    if (defined('Auth\OpenID\NO_MATH_SUPPORT')) {
        ...
        $message = Message::fromPostArgs($query_data);
        $session = PlainTextServerSession::fromMessage($message);
    } else {
    ...
    return KVForm::fromArray($reply_dict);
}

class TestFetcher extends \Auth\Yadis\HTTPFetcher {
    public function __construct($user_url, $user_page,
                                     $assoc_secret, $assoc_handle)
    {
        $this->get_responses = array($user_url =>
                                     new \Auth\Yadis\HTTPResponse($user_url,
                                                                     200,
                                                                     array(),
                                                                     $user_page));
    ...
    {
        if ($body === null) {
            return new \Auth\Yadis\HTTPResponse($url, 404, array(), 'Not found');
        } else {
            return new \Auth\Yadis\HTTPResponse($url, 200, array(), $body);
        }
    }
    ...
    public function _checkAuth($url, $body)
    {
        ...
        if ($query_data == $expected) {
            return new \Auth\Yadis\HTTPResponse($url, 200, array(), "is_valid:true\n");
        } else {
            return new \Auth\Yadis\HTTPResponse($url, 400, array(),
    ...
    function post($url, $body)
    {
        if (strpos($body, 'openid.mode=associate') !== false) {
            $response = associate($body, $this->assoc_secret,
                                              $this->assoc_handle);
            $this->num_assocs++;
...
class Tests_Auth_OpenID_Consumer extends PHPUnit_TestCase {
	...
	$query = \Auth\OpenID\parse($parsed['query']);
        ...
        $this->assertEquals(\Auth\OpenID\SUCCESS, $result->status);
    ...
    private function _test_success($user_url, $delegate_url, $links, $immediate = false)
    {
        ...
        $fetcher = new \Auth\OpenID\TestFetcher($user_url, $user_page,
                                               $_Auth_OpenID_assocs[0][0],
                                               $_Auth_OpenID_assocs[0][1]);

        $consumer = new \Auth\OpenID\GenericConsumer($store);
...
class ConfigurableConsumer extends \Auth\OpenID\GenericConsumer {
...
class Tests_Auth_OpenID_Consumer_TestSetupNeeded extends _TestIdRes {
    function failUnlessSetupNeeded($expected_setup_url, $message)
    {
        if ($this->consumer._checkSetupNeeded($message)) {
            $this->assertEquals($expected_setup_url,
                                $message->getArg(\Auth\OpenID\OPENID_NS,
                                                 'user_setup_url'));
        } else {
    ...
    function test_setupNeededOpenID1()
    {
        ...
        $message = \Auth\OpenID\Message::fromPostArgs(array(
            'opaenid.mode' => 'id_res',
    ...
    function test_noSetupNeededOpenID1()
    {
        ...
        // checkid_immediate
        $message = \Auth\OpenID\Message::fromOpenIDArgs(array('mode' => 'id_res'));
        $this->assertTrue($message->isOpenID1());
    ...
    {
        $message = \Auth\OpenID\Message::fromOpenIDArgs(array(
            'mode' => 'setup_needed',
...
class IdResCheckForFieldsTest extends _TestIdRes {
  function setUp() {
    # Argh.
    $v = null;
    $this->consumer = new \Auth\OpenID\GenericConsumer($v);
  }

  function successTest($openid_args, $signed_list) {
    $message = \Auth\OpenID\Message::fromOpenIDArgs($openid_args);
    $message->setArg(\Auth\OpenID\OPENID_NS, 'signed', implode(',', $signed_list));
    $result = $this->consumer->_idResCheckForFields($message);
    $this->assertFalse(\Auth\OpenID::isFailure($result));
  }
  ...
  function missingSignedTest($openid_args) {
    ...
    $result = $this->consumer->_idResCheckForFields($message);
    $this->assertTrue(\Auth\OpenID::isFailure($result));
    if (\Auth\OpenID::isFailure($result)) {
...
class GoodAssociation {
    ...
    {
        global $GOODSIG;
        return $message->getArg(\Auth\OpenID\OPENID_NS, 'sig') == $GOODSIG;
    }
}
...
class TestIdResCheckSignature extends _TestIdRes {
    function setUp()
    {
        ...
        $this->message = \Auth\OpenID\Message::fromPostArgs(array(
            'openid.mode'=> 'id_res',
    ...
    {
        $this->message.setArg(\Auth\OpenID\OPENID_NS, 'sig', 'BAD SIGNATURE');
...
class Tests_Auth_OpenID_Stateless1 extends _TestIdRes {
    ...
    function test_stateless()
    {
        ...
        // goodthings
        $this->message->setArg(\Auth\OpenID\OPENID_NS, "assoc_handle", "dumbHandle");
        $this->consumer->_idResCheckSignature($this->message,
                                              $this->endpoint->server_url);
    }
}
...
class Tests_Auth_OpenID_Stateless2 extends _TestIdRes {
    ...
    function test_statelessRaisesError()
    {
        // assoc_handle missing assoc, consumer._checkAuth returns
        // goodthings
        $this->message->setArg(\Auth\OpenID\OPENID_NS, "assoc_handle",
                               "dumbHandle");
...
class Tests_Auth_OpenID_Consumer_CheckNonceTest extends _TestIdRes {
    ...
    function test_openid1Success()
    {
        ...
        $nonce_value = \Auth\OpenID\mkNonce();
        $this->return_to = sprintf('http://rt.unittest/?nonce=%s',
                                   $nonce_value);
        $this->response = \Auth\OpenID\Message::fromOpenIDArgs(
                            array('return_to' => $this->return_to));
        $this->response->setArg(\Auth\OpenID\BARE_NS, 'nonce', $nonce_value);
    ...
    {
        // use consumer-generated nonce
        $this->response = \Auth\OpenID\Message::fromOpenIDArgs(array());
    ...
    function test_serverNonce()
    {
        ...
        $this->response = \Auth\OpenID\Message::fromOpenIDArgs(
                            array('ns' => \Auth\OpenID\OPENID2_NS,
                                  'response_nonce' => \Auth\OpenID\mkNonce()));
    ...
    function test_badNonce()
    {
        ...
        $nonce = \Auth\OpenID\mkNonce();
        list($timestamp, $salt) = \Auth\OpenID\splitNonce($nonce);

        $this->store->useNonce($this->server_url, $timestamp, $salt);

        $response = \Auth\OpenID\Message::fromOpenIDArgs(array(
...
class Tests_Auth_OpenID_Consumer_TestCheckAuthTriggered extends _TestIdRes {
    ...
    function test_checkAuthTriggeredWithAssoc()
    {
        ...
        $issued = time();
        $lifetime = 1000;
        $assoc = new \Auth\OpenID\Association(
...
class Tests_Auth_OpenID_Complete extends _TestIdRes {
	...
	$this->consumer->disableReturnToChecking();
        ...
        $this->assertEquals($r->status, \Auth\OpenID\CANCEL);
        $this->assertTrue($r->identity_url == $this->endpoint->claimed_id);
    }

    function test_cancel_with_return_to() {
        $message = \Auth\OpenID\Message::fromPostArgs(array('openid.mode' => 'cancel'));
    ...
    function test_errorWithNoOptionalKeys()
    {
        ...
        $message = \Auth\OpenID\Message::fromPostArgs(array('openid.mode'=> 'error',
                 'openid.error'=> $msg,
	...
	$this->consumer->disableReturnToChecking();
        ...
        $r = $this->consumer->complete($message, $this->endpoint, null);
        $this->assertEquals($r->status, \Auth\OpenID\FAILURE);
        $this->assertTrue($r->identity_url == $this->endpoint->claimed_id);
    ...
    function test_errorWithOptionalKeys()
    {
        ...
        $message = \Auth\OpenID\Message::fromPostArgs(array('openid.mode'=> 'error',
                 ...
                 'openid.contact'=> $contact, 'openid.ns'=> \Auth\OpenID\OPENID2_NS
                                                           ));
        $r = $this->consumer->complete($message, $this->endpoint, null);
        $this->assertEquals($r->status, \Auth\OpenID\FAILURE);
        $this->assertTrue($r->identity_url == $this->endpoint->claimed_id);
    ...
    function test_idResMissingField()
    {
        ...
        $message = \Auth\OpenID\Message::fromPostArgs($query);
        $r = $this->consumer->complete($message, $this->endpoint, null);
        $this->assertTrue(\Auth\openID::isFailure($r));
    }
}

class _VerifiedError extends \Auth\OpenID\FailureResponse {
}
...
class SetupNeededConsumer extends \Auth\OpenID\GenericConsumer {
...
class Tests_Auth_OpenID_SetupNeeded extends _TestIdRes {
    function test_setupNeededIdRes()
    {
        $message = \Auth\OpenID\Message::fromOpenIDArgs(array('mode'=> 'id_res'));
        $response = $this->consumer->complete($message, null, null);
        $this->assertEquals(\Auth\OpenID\SETUP_NEEDED,
                            $response->status);
    }
}
...
class TestCompleteMissingSig extends PHPUnit_TestCase {
    ...
    function setUp()
    {
        ...
        $this->message = \Auth\OpenID\Message::fromOpenIDArgs(
        array('mode'=> 'id_res',
    ...
    {
        $this->message->delArg(\Auth\OpenID\OPENID_NS, 'identity');
        $this->message->delArg(\Auth\OpenID\OPENID_NS, 'claimed_id');
        $this->endpoint->claimed_id = null;
        $this->message->setArg(\Auth\OpenID\OPENID_NS,
    ...
    {
        $this->message->setArg(\Auth\OpenID\OPENID_NS, 'signed',
    ...
    {
        if ($response->status != \Auth\OpenID\SUCCESS) {
...
class TestReturnToArgs extends PHPUnit_TestCase {
    ...
    {
        $store = null;
        $this->consumer = new \Auth\OpenID\GenericConsumer($store);
    }
    ...
    function test_returnToArgsUnexpectedArg()
    {
        ...
        // exceptions.
        $this->assertTrue(\Auth\OpenID::isFailure(
    ...
    function test_completeBadReturnTo()
    {
        ...
        $m = new \Auth\OpenID\Message(\Auth\OpenID\OPENID1_NS);
        $m->setArg(\Auth\OpenID\OPENID_NS, 'mode', 'cancel');
        $m->setArg(\Auth\OpenID\BARE_NS, 'foo', 'bar');
        $endpoint = null;

        foreach ($bad_return_tos as $bad) {
            $m->setArg(\Auth\OpenID\OPENID_NS, 'return_to', $bad);
    ...
    function test_completeGoodReturnTo()
    {
        ...
        $good_return_tos = array(
                                 ...
                                 array($return_to . "?another=arg",
                                       array(array(
                                                   array(\Auth\OpenID\BARE_NS, 'another'),
                                                   'arg'))),
        ...
        foreach ($good_return_tos as $pair) {
            ...
            $m->setArg(\Auth\OpenID\OPENID_NS, 'return_to', $good_return_to);
            $result = $this->consumer->complete($m, $endpoint, $return_to);
            $this->assertTrue(is_a($result, '\Auth\OpenID\CancelResponse'));
        }
    }
}
...
class Tests_Auth_OpenID_CheckAuthResponse extends _TestIdRes {
    ...
    function test_goodResponse()
    {
        ...
        $message = \Auth\OpenID\Message::fromOpenIDArgs($response);
    ...
    function test_missingAnswer()
    {
        ...
        // answer
        $response = array();
        $message = \Auth\OpenID\Message::fromPostArgs($response);
        $r = $this->consumer->_processCheckAuthResponse($message, $this->server_url);
        $this->assertFalse($r);
    }
...
class _IdResFetchFailingConsumer extends \Auth\OpenID\GenericConsumer {
    public $message = 'fetch failed';

    public function _doIdRes($message, $endpoint, $return_to=null)
    {
        return new \Auth\OpenID\FailureResponse($endpoint,
                                               $this->message);
    }
}
...
class _ExceptionRaisingMockFetcher {
    ...
    function post($url, $body)
    {
        ...
        return new \Auth\Yadis\HTTPResponse($url, 400,
                                               array(), '');
    }
}

class _BadArgCheckingConsumer extends \Auth\OpenID\GenericConsumer {
...
class Tests_Auth_OpenID_Consumer_TestCheckAuth extends _TestIdRes {
    ...
    {
        global $_Auth_OpenID_server_url;
        $this->fetcher->response = new \Auth\Yadis\HTTPResponse("http://some_url",
                                                                404,
                                                                array(),
                                                                "blah:blah\n");
    ...
    function test_signedList()
    {
        ...
        $query = \Auth\OpenID\Message::fromOpenIDArgs(array(
            'mode'=> 'id_res',
            'sig'=> 'rabbits',
            'identity'=> '=example',
    ...
    function test_112()
    {
        ...
        $this->assertEquals(\Auth\OpenID\OPENID2_NS, $args['openid.ns']);
        $incoming = \Auth\OpenID\Message::fromPostArgs($args);
        ...
        $expected = \Auth\OpenID\Message::fromPostArgs($expected_args);
...
class Tests_Auth_OpenID_Consumer_TestFetchAssoc extends PHPUnit_TestCase {
    ...
    function test_error_404()
    {
        ...
        $this->fetcher->response = new \Auth\Yadis\HTTPResponse(
           "http://some_url", 404, array('Hea'=> 'der'), 'blah:blah\n');

        $result = $this->consumer->_makeKVPost(
               \Auth\OpenID\Message::fromPostArgs(array('mode'=>'associate')),
               "http://server_url");
...
class Tests_Auth_OpenID_AuthRequestHTMLMarkup extends PHPUnit_TestCase {
    function setUp()
    {
        ...
        $this->request = new \Auth\OpenID\AuthRequest($this->endpoint, null);
    }
...
class Tests_Auth_OpenID_SuccessResponse extends PHPUnit_TestCase {
    ...
    function test_extensionResponse()
    {
        ...
        $message = \Auth\OpenID\Message::fromPostArgs($query);
        $resp = new \Auth\OpenID\SuccessResponse($this->endpoint, $message);

        $utargs = $resp->extensionResponse($uri, false);
        $this->assertEquals($utargs, array('one' => '1', 'two' => '2'));
        $sregargs = $resp->extensionResponse(\Auth\OpenID\SREG_NS_URI, false);
    ...
    function test_extensionResponseSigned()
    {
        ...
        $msg = \Auth\OpenID\Message::fromOpenIDArgs($args);
        $resp = new \Auth\OpenID\SuccessResponse($this->endpoint, $msg, $signed_list);
    ...
    function test_returnTo()
    {
        ...
        $resp = new \Auth\OpenID\SuccessResponse($this->endpoint,
...
class _StubConsumer {
    ...
    {
        $auth_req = new \Auth\OpenID\AuthRequest($service, $this->assoc);
...
class Tests_Auth_OpenID_DiscoFailure extends PHPUnit_TestCase {
    ...
    function setUp()
    {
        ...
        $this->store = null;
        $this->session = new \Auth\Yadis\PHPSession();
        $cls = $this->consumerClass;
        $this->consumer = new $cls($this->store, $this->session);
        $this->consumer->consumer = new _StubConsumer();
        $this->discovery = new \Auth\Yadis\Discovery($this->session,
                                         $this->claimed_id,
                                         $this->consumer->session_key_prefix);
    }
}

class Consumer_completeEmptySession extends \Auth\OpenID\GenericConsumer {
    ...
    function complete($message, $endpoint, $return_to)
    {
        ...
        return new \Auth\OpenID\FailureResponse($endpoint, $this->text);
    }
}
...
class Tests_Auth_OpenID_ConsumerTest2 extends PHPUnit_TestCase {
    function setUp()
    {
        ...
        $this->store = null;
        $this->session = new \Auth\Yadis\PHPSession();
        $this->consumer = new \Auth\OpenID\Consumer($this->store, $this->session);
        $this->consumer->consumer = new _StubConsumer();
        $this->discovery = new \Auth\Yadis\Discovery($this->session,
                                         $this->claimed_id,
                                         $this->consumer->session_key_prefix);
    ...
    function test_beginWithoutDiscovery()
    {
        ...
        $loader = new \Auth\OpenID\ServiceEndpointLoader();
    ...
    function test_completeEmptySession()
    {
        ...
        $this->assertTrue(\Auth\OpenID::isFailure($response));
    ...
    private function _doResp($auth_req, $exp_resp)
    {
        ...
        if ($this->endpoint->claimed_id != \Auth\OpenID\IDENTIFIER_SELECT) {
    ...
    function test_successDifferentURL()
    {
        ...
        $this->endpoint->claimed_id = $this->endpoint->local_id = \Auth\OpenID\IDENTIFIER_SELECT;

        // Use a response endpoint with a different URL (asserted by
        // the IDP)
        $resp_endpoint = new \Auth\OpenID\ServiceEndpoint();
    ...
    {
        $message = \Auth\OpenID\Message::fromPostArgs(array());
        $this->_doRespNoDisco(new \Auth\OpenID\SuccessResponse($this->endpoint,
                                                              $message));
    }

    function test_noDiscoCompleteCancelWithToken()
    {
        $this->_doRespNoDisco(new \Auth\OpenID\CancelResponse($this->endpoint));
    }

    function test_noDiscoCompleteFailure()
    {
        $msg = 'failed!';
        $resp = $this->_doRespNoDisco(new \Auth\OpenID\FailureResponse($this->endpoint, $msg));
    ...
    function test_noDiscoCompleteSetupNeeded()
    {
        ...
        $resp = $this->_doRespNoDisco(
              new \Auth\OpenID\SetupNeededResponse($this->endpoint, $setup_url));
    ...
    function test_completeSuccess()
    {
        ...
        $this->_doRespDisco(true,
                            new \Auth\OpenID\SuccessResponse($this->endpoint,
                                                            $message));
    }
    ...
    {
        $this->_doRespDisco(true,
                            new \Auth\OpenID\CancelResponse($this->endpoint));
    }
    ...
    function test_completeFailure()
    {
        ...
        $resp = $this->_doRespDisco(false,
                    new \Auth\OpenID\FailureResponse($this->endpoint, $msg));
        $this->assertTrue($resp->message == $msg);
    }
    ...
    function test_completeSetupNeeded()
    {
        ...
        $resp = $this->_doRespDisco(false,
            new \Auth\OpenID\SetupNeededResponse($this->endpoint, $setup_url));
        $this->assertTrue($resp->status == \Auth\OpenID\SETUP_NEEDED);
        $this->assertTrue($resp->setup_url == $setup_url);
    }
...
class IDPDrivenTest_Consumer2 extends ConfigurableConsumer {
    ...
    {
        return new \Auth\OpenID\FailureResponse(null,
                                               "Bogus");
    }
}
...
class TestDiscoveryVerification_test_otherServer extends \Auth\OpenID\GenericConsumer {
    private function _discoverAndVerify($to_match, $to_match_endpoints=null)
    {
        global $__test_otherServer_text;
        return new \Auth\OpenID\FailureResponse(null, $__test_otherServer_text);
    }
}
...
class TestCreateAssociationRequest extends PHPUnit_TestCase {
    function setUp()
    {
        ...
        $s = null;
        $this->consumer = new \Auth\OpenID\GenericConsumer($s);
    ...
    function test_noEncryptionSendsType()
    {
        ...
        $this->assertTrue(is_a($session, '\Auth\OpenID\PlainTextConsumerSession'));

        $expected = \Auth\OpenID\Message::fromOpenIDArgs(
                                                        array('ns' => \Auth\OpenID\OPENID2_NS,
    ...
    function test_noEncryptionSendsTypeHMACSHA256()
    {
        ...
        $response = \Auth\OpenID\Message::fromOpenIDArgs(
                                                        array('ns' => \Auth\OpenID\OPENID2_NS,
                                                              'session_type'=>$session_type,
                                                              'assoc_type'=>$this->assoc_type,
        ...
        $this->assertTrue(is_a($assoc, '\Auth\OpenID\Association'));
    ...
    function test_noEncryptionCompatibility()
    {
        ...
        $this->assertTrue(is_a($session, '\Auth\OpenID\PlainTextConsumerSession'));
        $this->assertEquals(\Auth\OpenID\Message::fromOpenIDArgs(array('mode'=>'associate',
                                                                      'assoc_type'=>$this->assoc_type)),
                            $args);
    }
    ...
    function test_dhSHA1Compatibility()
    {
        ...
        $this->assertTrue(is_a($session,
                               '\Auth\OpenID\DiffieHellmanSHA1ConsumerSession'));

        // This is a random base-64 value, so just check that it's
        // present.
        $this->assertTrue($args->hasKey(\Auth\OpenID\OPENID1_NS, 'dh_consumer_public'));
        $args->delArg(\Auth\OpenID\OPENID1_NS, 'dh_consumer_public');

        // OK, session_type is set here and not for no-encryption
        // compatibility
        $expected = \Auth\OpenID\Message::fromOpenIDArgs(array('mode'=>'associate',
                                                              'session_type'=>'DH-SHA1',
                                                              'assoc_type'=>$this->assoc_type,
                                                              'dh_modulus'=> 'BfvStQ==',
                                                              'dh_gen'=> 'Ag=='));
...
class TestDiffieHellmanResponseParameters extends PHPUnit_TestCase {
    ...
    function setUp()
    {
        ...
        $this->server_dh = new \Auth\OpenID\DiffieHellman(100389557, 2);
        $this->consumer_dh = new \Auth\OpenID\DiffieHellman(100389557, 2);
        ...
        $this->secret = \Auth\OpenID\CryptUtil::randomString(
        ...
        $this->msg = new \Auth\OpenID\Message($this->message_namespace);
    }

    function testExtractSecret()
    {
        $this->msg->setArg(\Auth\OpenID\OPENID_NS, 'dh_server_public',
                           $this->dh_server_public);

        $this->msg->setArg(\Auth\OpenID\OPENID_NS, 'enc_mac_key',
                           $this->enc_mac_key);
...
class TestOpenID1SHA1 extends TestDiffieHellmanResponseParameters {
    public $session_cls = '\Auth\OpenID\DiffieHellmanSHA1ConsumerSession';
    public $message_namespace = \Auth\OpenID\OPENID1_NS;
}

class TestOpenID2SHA1 extends TestDiffieHellmanResponseParameters {
    public $session_cls = '\Auth\OpenID\DiffieHellmanSHA1ConsumerSession';
    public $message_namespace = \Auth\OpenID\OPENID2_NS;
}

if (!defined('Auth\OpenID\NO_MATH_SUPPORT') &&
    \Auth\OpenID\SHA256_SUPPORTED) {
    class TestOpenID2SHA256 extends TestDiffieHellmanResponseParameters {
        public $session_cls = '\Auth\OpenID\DiffieHellmanSHA256ConsumerSession';
        public $message_namespace = \Auth\OpenID\OPENID2_NS;
    }
}
...
class Tests_Auth_OpenID_KVPost extends PHPUnit_TestCase {
    ...
    function test_200()
    {
        $response = new \Auth\Yadis\HTTPResponse();
        $response->status = 200;
        $response->body = "foo:bar\nbaz:quux\n";
        $r = \Auth\OpenID\GenericConsumer::_httpResponseToMessage($response, $this->server_url);
        $expected_msg = \Auth\OpenID\Message::fromOpenIDArgs(array('foo' => 'bar', 'baz' => 'quux'));
    ...
    function test_400()
    {
        ...
        $result = \Auth\OpenID\GenericConsumer::_httpResponseToMessage($response, $this->server_url);

        $this->assertTrue(is_a($result, '\Auth\OpenID\ServerErrorContainer'));
